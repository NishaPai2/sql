...
FROM (
    SELECT
        ...
        COUNT(*) OVER (
            ...
            ORDER BY market_date  -- running count
        ) AS purchase_count
    FROM customer_purchases
    WHERE market_date < '2022-04-29'
) counted_purchases;
...
--QUERY 7 - I tried a code based on HINT in instructions
...
-- Step 2: flag the best and worst days
flagged_sales AS (
    SELECT
        market_date,
        total_sales,
        CASE 
            WHEN total_sales = (SELECT MAX(total_sales) FROM total_sales_by_date) THEN 'Highest'
            WHEN total_sales = (SELECT MIN(total_sales) FROM total_sales_by_date) THEN 'Lowest'
        END AS type
...
--QUERY 8 - code with CROSS JOIN as subquery
SELECT 
    vendor_name,
    product_name,
    SUM(original_price * 5) AS total_sales
FROM (
    -- Subquery: explode all vendor-product-customer combinations
    SELECT 
        v.vendor_name,
        p.product_name,
        vi.original_price
...
--QUERY 8 - to check product of cross join queries before group by 
SELECT COUNT(*) AS exploded_rows
FROM vendor v
CROSS JOIN product p
CROSS JOIN customer c;
--END QUERY
...
VALUES (
    101,                 -- product_id (choose a unique number)
    'Apple Pie',         -- product_name
    '1 slice',           -- product_size
    5,                   -- product_category_id (choose an appropriate category)
